<div align="center">

<img src="assets/logo.svg" alt="Hery RAKOTONARIVO" width="220">

```text
Greetings to each and every one of you

hery@madagascar:~$ whoami

██      ██                                    
██      ██                                    
██      ██    ██████    ██  ████    ██      ██
██████████  ██      ██  ████    ██  ██      ██
██      ██  ██████████  ██          ██      ██
██      ██  ██          ██            ████████
██      ██    ██████    ██                  ██
                                            ██
R  A  K  O  T  O  N  A  R  I  V  O  ████████  

software engineer · backend & databases · open for freelance
```

[![committers.top badge](https://user-badge.committers.top/madagascar_private/Hery0019.svg)](https://user-badge.committers.top/madagascar_private/Hery0019)

</div>

### `~/about`

```text
location   Antananarivo, Madagascar · remote
focus      business applications, databases, and AI in the tools teams already use
languages  French, English
site       imhery.dev
reply      within 24 hours
```

### `~/projects`

```text
prepwork       ts · spring · react    a scaffolding CLI, plus the specs for the agent inside it
Ledger         c++20 · cmake          an embedded SQL engine with plain-text storage
huffman        java · servlet · pg    Huffman coding, step by step
MobileCrypto   react native · expo    a crypto wallet: auth, portfolio, transactions, charts
chess          java · swing           a full chess game, no framework
BoulangeHery   java · servlet · pg    a bakery: products, stock, sales
```

[`prepwork`](https://github.com/Hery0019/prepwork) ·
[`Ledger`](https://github.com/Hery0019/Ledger) ·
[`huffman`](https://github.com/Hery0019/huffman) ·
[`MobileCrypto`](https://github.com/Hery0019/MobileCrypto) ·
[`chess`](https://github.com/Hery0019/chess) ·
[`BoulangeHery`](https://github.com/Hery0019/BoulangeHery) ·
[`all repositories ↗`](https://github.com/Hery0019?tab=repositories)

### `~/stack`

```text
backend    java · spring boot · node · php
data       postgresql · mysql · oracle
front      react · next.js · typescript · tailwind
tooling    docker · git · prompt engineering
```

### `~/contributions`

<!-- The snake is loaded through jsDelivr, pinned to a commit of the output branch, on purpose.
     GitHub proxies external images through camo, whose Fastly backend is not the one behind
     raw.githubusercontent.com: measured from Antananarivo (Fastly Johannesburg POP) on 9 Sept. 2026,
     raw answered 503 "Backend.max_conn reached" on 91-100 % of its cache misses while camo fetched
     the same files 146/146 times at the same seconds. The commit sha below is rewritten by
     .github/workflows/snake.yml each time the snake changes (one bot commit on main), so every
     cache and browser picks up the new drawing at the next page load. Do not edit the sha by hand
     and do not add another URL of the form cdn.jsdelivr.net/gh/Hery0019/Hery0019@<sha> to this file.
     Rollback (works everywhere except through the Johannesburg POP): put back
     https://raw.githubusercontent.com/Hery0019/Hery0019/output/github-contribution-grid-snake.svg
     and https://raw.githubusercontent.com/Hery0019/Hery0019/output/github-contribution-grid-snake-dark.svg -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://cdn.jsdelivr.net/gh/Hery0019/Hery0019@94f7276d0ac004001741ecce50a312ac498f67fc/github-contribution-grid-snake-dark.svg"/>
  <source media="(prefers-color-scheme: light)" srcset="https://cdn.jsdelivr.net/gh/Hery0019/Hery0019@94f7276d0ac004001741ecce50a312ac498f67fc/github-contribution-grid-snake.svg"/>
  <img src="https://cdn.jsdelivr.net/gh/Hery0019/Hery0019@94f7276d0ac004001741ecce50a312ac498f67fc/github-contribution-grid-snake-dark.svg" alt="Contribution snake" width="100%"/>
</picture>

### `~/contact`

```console
$ mail herakotonarivo@gmail.com
$ open wa.me/261380732115
$ open linkedin.com/in/herakotonarivo
$ open imhery.dev
```

[`email`](mailto:herakotonarivo@gmail.com) ·
[`whatsapp`](https://wa.me/261380732115) ·
[`linkedin`](https://www.linkedin.com/in/herakotonarivo/) ·
[`imhery.dev`](https://imhery.dev)
